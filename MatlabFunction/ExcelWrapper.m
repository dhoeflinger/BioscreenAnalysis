function  ExcelWrapper( excel_file_name, excel_output_file, max_timepoint)
...
for i=1:dims(2)
    ...
    if (strcmpi(char(title_data(2,i)),'Time'))
        ...
        if (max_timepoint < 0)
            [lagtime, max_u, OD_max, doubling_time, note] = MicrobialKinetics(Data(:,i), time_interval);
        else
            [lagtime, max_u, OD_max, doubling_time, note] = MicrobialKinetics(Data(1:max_timepoint/time_interval,i), time_interval);
        end
        lag_times(i) = lagtime;
...
for j = 1:sugar_count
    ...
    for k = start_idx:start_idx + count-1
        subplot(count, sugar_count, (k-start_idx) * sugar_count + j)
        if (max_timepoint < 0)
